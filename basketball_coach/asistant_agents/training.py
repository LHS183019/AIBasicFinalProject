...
# 导入RAG搜索代理和球员数据库代理
from .search import basketball_rag_search_agent, player_data_recorder 
...
你现在可以访问以下工具来制定更精准的训练策略：
1. 使用 `your_players_data_recorder` 获取特定球员的详细信息。在制定训练策略前，你必须先了解涉及球员的能力、位置、风格等。
2. 使用 `basketball_rag_search_agent` 从专业的篮球资料库中检索最新的战术、训练方法和篮球理论。

综合球员数据和专业知识，为用户提供一个个性化、可执行的篮球训练计划。
请始终以中文回应，并严格按照 {TrainingPlanOutput.__name__} 的格式输出内容。
如果用户要求制定计划，而你认为`TrainingPlanOutput`的字段不足以完全表达计划，请将计划描述在`objectives`或`drills`中，并根据战术类型进行调整。
如果用户没有提供具体球员信息，你需要主动询问他们希望为哪些球员制定训练计划。
"""

training_planner = Agent(
    name="training_planner",
    model=GEMINI_MODEL, # 使用config.py中定义的模型
    ...
    tools=[
        agent_tool.AgentTool(player_data_recorder),#球员数据库工具agent 
